...
4. At eval time, the wrapper:
   ...
   - and only then fuses the retrieval distribution into the model logits.
   - otherwise it falls back to the original model logits unchanged.
...
`train.py` in this repo saves a raw `state_dict` by default, so the eval script needs the same model architecture flags (`--model`, `--d_model`, `--n_layers`, and any variant-specific args) in order to rebuild the checkpointed network before attaching the sidecar.
